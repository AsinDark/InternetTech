...
class App {
  ...
  onInputKeydown = async ({ key, target }) => {
    if (key === 'Enter') {
      if (target.value) {
        const re = /([Районный, Магистральный, Социальный][1-9]*)/g;
        if (!(target.value.match(re) || []).length) {
          alert("Неверный формат!\nФормат записи номера маршрута:\n [Районный, Магистральный, Социальный] число")
          return;
        }
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    let newTaskName = prompt('Добавить остановку');
